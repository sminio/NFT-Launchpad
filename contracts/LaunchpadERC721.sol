...
import "./lib/ERC721Logic.sol";
import "erc721a-upgradeable/contracts/ERC721AUpgradeable.sol";
import "erc721a-upgradeable/contracts/ERC721A__Initializable.sol";
import "erc721a-upgradeable/contracts/extensions/ERC721AQueryableUpgradeable.sol";
import "./lib/ERC721A__OwnableUpgradeable.sol";
import "solady/src/utils/LibString.sol";
import "closedsea/src/OperatorFilterer.sol";
import "@openzeppelin/contracts-upgradeable/token/common/ERC2981Upgradeable.sol";

contract LaunchpadERC721 is
  ...
  ERC721AQueryableUpgradeable
{
  ...
  function batchMintTo(
    ...
    ERC721PriceUtils.validateMint(invite, config, auth, _minted, signature, args);
    ...
    ERC721PriceUtils.updateBalances(
      invite,
      config,
      _ownerBalance,
      _affiliateBalance,
  ...
  function mintTo(
    ...
    ERC721PriceUtils.validateMint(i, config, auth, _minted, signature, args);
    _mint(to, quantity);
  ...
  function burnToMint(uint256[] calldata tokenIds) external {
    ...
    ERC721PriceUtils.validateBurnToMint(config, burnConfig, tokenIds, curSupply, _minted);
  ...
  function tokenURI(
    ...
    return
      bytes(config.baseUri).length != 0
        ? string(abi.encodePacked(config.baseUri, LibString.toString(tokenId), ".json"))
        : "";
  }
  ...
  function withdrawTokens(address[] memory tokens) public {
    ERC721PriceUtils.withdrawTokens(config, _ownerBalance, _affiliateBalance, owner(), tokens);
  }
  ...
  function computePrice(
    ...
    return ERC721PriceUtils.computePrice(i, config.discounts, quantity, listSupply_, affiliateUsed);
  }
